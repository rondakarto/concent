...
var game = cc.Layer.extend({
    ...
    init:function () {
        this._super();
        var gradiant = new cc.LayerGradient( cc.color(0,0,0,255), cc.color(0x46,0x82,0xB4,255));
        this.addChild( gradiant );
        scoreText = new cc.LabelTTF( "Moves: 0", "Arial", "32", cc.TEXT_ALIGNMENT_CENTER );
            ...
            if(pickedTiles.length < 2) {
                ...
                if (cc.rectContainsPoint(targetRectangle, location)) {
                    if( pickedTiles.indexOf(target) == -1) {
                        var pic = "tile_" + target.pictureValue + "_png"
                        target.initWithFile(res[pic]);
